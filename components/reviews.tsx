...
export function Reviews({ reviews: initialReviews }: ReviewsProps) {
      ...
      <section ref={sectionRef} className="py-20 bg-[#0a0a0a]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <div className="py-16">
              <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-[#6b7280]/10 mb-4 animate-pulse">
  ...
  return (
    <section ref={sectionRef} className="py-20 bg-[#0a0a0a] relative overflow-hidden">
      {/* Background accents */}
      <div className="absolute top-0 left-1/4 w-[600px] h-[600px] bg-[#6b7280]/5 blur-[120px] rounded-full" />
        ...
        >
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="group relative w-12 h-12 rounded-full bg-[#6b7280]/10 hover:bg-[#6b7280]/20 border border-[#6b7280]/20 hover:border-[#6b7280]/40 flex items-center justify-center transition-all duration-300 hover:scale-110 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <button
              onClick={nextSlide}
              disabled={isTransitioning}
              className="group relative w-12 h-12 rounded-full bg-[#6b7280] hover:bg-[#9ca3af] flex items-center justify-center transition-all duration-300 hover:scale-110 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed overflow-hidden"
            >
            ...
            return (
              <div
                ...
                className={`group relative bg-[#111111] border-2 rounded-2xl p-6 transition-all duration-700 ${
                  ...
                  isHovered 
                    ? "border-[#6b7280]/70 -translate-y-2 shadow-xl shadow-[#6b7280]/20" 
                    : "border-[#262626]"
                }`}
              ...
              >
                {/* Top accent line */}
                <div
                  className={`absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[#6b7280] via-[#9ca3af] to-[#6b7280]/50 rounded-t-2xl transition-all duration-500 ${
                ...
                <div className={`mb-4 transition-all duration-500 ${
                  ...
                  <Quote className="w-10 h-10 text-[#6b7280]/40" />
                </div>
                ...
                {/* Divider */}
                <div className={`h-px bg-gradient-to-r from-transparent via-[#6b7280]/30 to-transparent mb-4 transition-all duration-500 ${
                  isHovered ? "via-[#6b7280]/60" : ""
                }`} />

                {/* Author info */}
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`relative w-10 h-10 rounded-full bg-[#6b7280] flex items-center justify-center text-white font-bold transition-all duration-500 overflow-hidden ${
                      isHovered ? "scale-110 shadow-lg shadow-[#6b7280]/40" : ""
                    }`}>
                      {/* Ring effect */}
                      <div className={`absolute inset-0 rounded-full border-2 border-[#6b7280] transition-all duration-500 ${
                  ...
                  <div className="flex items-center gap-1.5">
                    <span className={`text-white font-bold transition-all duration-300 ${
                      isHovered ? "scale-110 text-[#6b7280]" : ""
                    }`}>
                      {review.rating}
                    </span>
          ...
          {Array.from({ length: Math.max(1, reviews.length - 2) }).map((_, i) => (
            <button
              ...
              className={`h-1.5 rounded-full transition-all duration-500 ${
                i === currentIndex 
                  ? "w-8 bg-[#6b7280] shadow-lg shadow-[#6b7280]/50" 
