...
/**
 * Fix Orders with Correct Schema
 * Uses the correct amount_cents field that the orders table expects
 */
...
async function processPendingSession(session) {
  ...
  try {
    ...
    // Create order record with CORRECT schema (amount_cents is required)
    const orderData = {
      order_number: orderNumber,
      customer_email: session.customer_email,
      customer_name: 'Customer',
      amount_cents: Math.round(session.total * 100), // REQUIRED field in cents
      currency: 'USD',
      status: 'completed',
...
async function fixAllPendingOrders() {
  log('🔧 Starting Correct Schema Orders Fix', 'bold');
  log('=' .repeat(50), 'blue');
